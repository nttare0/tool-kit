...
function Field({ label, value, onChange, area = false, required = false, type = 'text', autoComplete }: { label: string; value: string | number; onChange: (value: string) => void; area?: boolean; required?: boolean; type?: string; autoComplete?: string }) {
  const props = { value, required, type, autoComplete, onChange: (event: ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => onChange(event.target.value), placeholder: label };
...
function AuthGate({ status, onAuthenticated }: { status: AuthStatus; onAuthenticated: () => void }) {
  ...
  return <Shell><main className="mx-auto flex min-h-[70vh] max-w-md items-center px-4 py-12 md:px-8"><form onSubmit={submit} className="w-full border border-border bg-card p-7 md:p-9"><BrandMark /><div className="mt-10 font-mono text-[10px] uppercase tracking-[.16em] text-muted-foreground">{mode === 'setup' ? 'First-time setup' : 'Private workspace'}</div><h1 className="mt-3 text-3xl font-semibold tracking-[-.06em]">{mode === 'setup' ? 'Create your admin login.' : 'Welcome back.'}</h1><p className="mt-3 text-sm leading-6 text-muted-foreground">{mode === 'setup' ? 'This creates the first admin account and stores it securely in SQLite.' : 'Log in to manage tools, categories, publishing, and preview images.'}</p><div className="mt-7 space-y-4"><Field label="Email" value={email} onChange={setEmail} type="email" autoComplete="email" required /><Field label="Password (8+ characters)" value={password} onChange={setPassword} type="password" autoComplete={mode === 'setup' ? 'new-password' : 'current-password'} required /></div>{message && <p className="mt-4 text-xs text-red-600">{message}</p>}<button disabled={busy} className="mt-7 h-11 w-full rounded-md bg-black px-4 py-3 text-sm font-medium text-white transition-opacity hover:opacity-80 disabled:opacity-50">{busy ? 'Working…' : mode === 'setup' ? 'Create admin account' : 'Log in'}</button>{status.configured && <button type="button" onClick={() => setMode(mode === 'login' ? 'setup' : 'login')} className="mt-4 w-full text-xs text-muted-foreground underline underline-offset-4">{mode === 'login' ? 'First admin setup' : 'Back to login'}</button>}</form></main></Shell>;
}
...
export default function Admin() {
  ...
  useEffect(() => {
    const generated = makePreviewUrl(toolForm.websiteUrl);
    if (!generated || (toolForm.previewUrl && !toolForm.previewUrl.startsWith('https://api.microlink.io/')) || generated === toolForm.previewUrl) return;
    const timer = window.setTimeout(() => setTool('previewUrl', generated), 500);
    return () => window.clearTimeout(timer);
  }, [toolForm.websiteUrl, toolForm.previewUrl]);
